				...
				<table name="T_HEADER">	
					<xsl:for-each select="orders:BGM">
						<row id="{position() - 1}">
							...
							<xsl:for-each select="parent::*/orders:Segment_group_1/orders:RFF/c:C506">
								<xsl:choose>
							...
							<field name="WAERK"><!-- Moneda -->
									<xsl:value-of select="parent::*/orders:Segment_group_7/orders:CUX/c:C504_-_-1/c:e6345"/>
							</field>
							<field name="ZTERM"><!-- Cond de pago -->
								<xsl:value-of select="parent::*/orders:Segment_group_8/orders:PAT/c:C112/c:e2009"/>
							</field>
							<field name="ZMODP"><!-- Modo Pago -->
								<xsl:value-of select="parent::*/orders:Segment_group_8/orders:PAT/c:C112/c:e2151"/>
							</field>
						</row>
					</xsl:for-each>
				</table>
